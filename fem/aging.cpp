...
//YZ:added 191217
int couter_yige=0;
std::ofstream stress_fout("stress.log");
...
void ppixel(int nx, int ny, int nz, int ns, int nphase, char filename[15])
{
	...
	std::cout << "read in file in ppixel " << filename << " nphase: " << nphase << std::endl;
...
void femat(int nx, int ny, int nz, int ns, double (&phasemod)[101][51][3], int nphase, int q)
{
	...
	*/
	...
	//b, C values are to consider energy jumps at boundaries
	...
	*/
		...
		for (int j = 1; j <= ny; j++)
		{
			for (int i = 1; i <= nx; i++)
			{
				...
				if (phasechange[m] == 1) //phasechange[m] is global
				{
					...
					//get the b value
					//see MANUAL page 23, table 7 for delta
						...
						for (int mm = 1; mm <= 8; mm++)
						{
							...
							for (int m3 = 1; m3 <= 3; m3++)
							{
								for (int m8 = 1; m8 <= 8; m8++)
								{
									...
									{
										sum = sum + delta[m8][m3] * dk[pix[m]][m8][m3][mm][nn][qtime - pixt[m]];//YZ: eq(11), global b array
										C = C + 0.5*delta[m8][m3] * dk[pix[m]][m8][m3][mm][nn][qtime - pixt[m]]
											* delta[mm][nn]; //YZ: eq(11), global constant C
									}
								}
							}
	...
	for (int k = 1; k <= nz; k++)
	{
		for (int j = 1; j <= ny; j++)
		{
			for (int i = 1; i <= nx; i++)
			{
				int m = nx * ny*(k - 1) + (j - 1)*nx + i;

				//if the pixel is viscoelastic
				if (pix[m] == 12) //phase 12 is CSH
				{

					//Adds up all previous time steps //YZ: what does it mean??
					for (qtime = pixt[m] + 1; qtime <= q - 1; qtime++)
					{
						...
						for (int i8 = 1; i8 <= 8; i8++)
						{//YZ: for csh phase, define delta matrix, for b,C
							...
							for (int mm = 1; mm <= 8; mm++)
							{
								...
								for (int m3 = 1; m3 <= 3; m3++)
								{
									for (int m8 = 1; m8 <= 8; m8++)
									{
										...
										C = C + 0.5 - delta[m8][m3] * dk[pix[m]][m8][m3][mm][nn][q - qtime + 1]
											* delta[mm][nn];
									}
								}
								b[ib[m][is[mm]]][nn] = b[ib[m][is[mm]]][nn] + sum;
...
//Subroutine computes the total energy, utot, and the gradient, gb = Au + b
...
void stress(int nx, int ny, int  nz, int ns, int q) //100,100,100,1000000,time step
{
	...
	*/
	...
	std::cout << "enter stress loop" << std::endl;
	...
	for (int k = 1; k <= nz; k++)
	{
		for (int j = 1; j <= ny; j++)
		{
			for (int i = 1; i <= nx; i++)
			{
				...
				*/
				//YZ: added to output voxel stress
				//std::cout << "voxel stress "<< str11 << "   " << str22 << "   " << str33 << "   " << str13 << "   " << str23 << "   " << str12<<std::endl;

				//YZ:added 191217, output 
				if (pix[m]==12){
					couter_yige=couter_yige+1;
					stress_fout << "CSH voxel stress" << " " << couter_yige << "   " <<i<<j<<k << str11 <<std::endl;
				}
	...
	std::cout << "finish stress loop" << std::endl;
...
//modulus function for viscoelastic materials
//Young's modulus of CSH (GPa), t is the loading time of each specific CSH pixel
...
int main()
{
	...
	*/
	...
	double t[3]; //time step should be set accordingly to Thames microstructure images
	t[1]=1.0;
	t[2]=1.3;
	t[3]=1.6;
	/*
	...
	*/
	...
	string prefilename[3]; //file number should be equal to number of Thames microstructure images
	char filename[15];
	...
	*/
	...
	for (int k = 1; k <= nz; k++)
	{
		for (int j = 1; j <= ny; j++)
		{
			...
			{
				int m = nx * ny * (k - 1) + nx * (j - 1) + i;
				phasechange[m] = 0; //initialize aging state
	...
	*/
	...
	phasemod[0][1][2] = 0.00;
	phasemod[1][1][1] = 0.0; //phase 1 is water, bulk modulus of water is not considered yet
	phasemod[1][1][2] = 0.00;
	...
	for (int i = 2; i <= step; i++)
	{	
		phasemod[12][i][1] = Fmodulus(t[i]-t[1]); //TODO, how to update for each CSH pixel that appears at each time step????
	}
	...
	*/
	...
	for (int q = 1; q <= step; q++)
	{
		...
		*/
		ppixel(nx, ny, nz, ns, nphase, filename); //YG: read in 1.txt

		std::cout<<"loop 2 "<<q<<std::endl;
		/*
		...
		*/
		assig(ns, nphase, prob);
				...
				for (int j = 1; j <= ny; j++)
				{
					for (int i = 1; i <= nx; i++)
					{
						...
						if (q>1)
						{
							if (pix[m] != pixstor[m])
							{
								phasechange[m] = 1;//YG: aging will occur
								pixt[m] = q - 1; //YG: ??
							}
						}
						pixstor[m] = pix[m]; //YG: defined at q=1
					}
				}
			}


			std::cout<<"loop 3 "<<q<<std::endl;
			...
			*/
			...
			kn[25] = -1;
			kn[26] = 1;
			kn[27] = 0; //YG: 27th is the middle self
				...
				for (int j = 1; j <= ny; j++)
				{
					for (int i = 1; i <= nx; i++)
					{
						...
						for (int n = 1; n <= 27; n++)
						{
							...
							int j1 = j + jn[n];
							int k1 = k + kn[n];
							if (i1<1) i1 = i1 + nx; //YG: periodic limit
							if (i1>nx) i1 = i1 - nx;
							if (j1<1) j1 = j1 + ny;
							if (j1>ny) j1 = j1 - ny;
							if (k1<1) k1 = k1 + nz;
							...
							ib[m][n] = m1; //YG: voxel m 's n-th neighbor is voxel m1 (m=1-1million, n=1-27, m1=1-1million)
						}
					}
				}
			}
			...
			for (int k = 1; k <= nz; k++)
			{
				for (int j = 1; j <= ny; j++)
				{
					for (int i = 1; i <= nx; i++)
					{
						...
						double x = 1.0*(i - 1); //YG: 1.0 is the size of 1 pixel, 1um. xyz are coordinates
						double y = 1.0*(j - 1);
						double z = 1.0*(k - 1);
						u[m][q][1] = x*exx + y*exy + z*exz; //YG: u is displacements vector
			...
			*/
			...
			fout << "modulus"<< "   "<< strxx << "   " << stryy << "   " << strzz << "   " << strxy << "   " << stryz << "   " << strxz << "   ";

			fout << "volume fraction"<< "   "<< prob[12]; //YZ: added to output volume fractions
			fout << "\n";
		}
