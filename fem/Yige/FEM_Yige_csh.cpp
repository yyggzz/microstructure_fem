...
double csh_modulus(double str11, double str22,double str33,double str13,double str23,double str12) {
	double str_mean = (str11+str22+str33+str13+str23+str12)/6;
	double ind = 1/(15-str_mean)-1/25; // when str_mean = -10, ind=0, when str_mean = 15, ind=infinity
	double csh_mod = 11.2 + 11.2 * exp(-0.2 * ind);
	return csh_mod;
}
...
*/
void femat(int nx, int ny, int nz, int ns, double (&phasemod)[1000001][51][3], int nphase, int q)
{
							...
							for (int m3 = 1; m3 <= 3; m3++)
							{
								for (int m8 = 1; m8 <= 8; m8++)
								{
									...
									{	//Question:???not a loop, only once
										sum = sum + delta[m8][m3] * dk[m][m8][m3][mm][nn][qtime - pixt[m]]; //eq(11), global b array
										C = C + 0.5 * delta[m8][m3] * dk[m][m8][m3][mm][nn][qtime - pixt[m]] * delta[mm][nn]; //eq(11), global constant C		 
									}
								}
							}
						...
						for (int nn = 1; nn <= 3; nn++)
						{
							for (int mm = 1; mm <= 8; mm++)
							{
								...
								for (int m3 = 1; m3 <= 3; m3++)
								{
									for (int m8 = 1; m8 <= 8; m8++)
									{
										sum = sum - delta[m8][m3] * dk[m][m8][m3][mm][nn][q - qtime + 1];
										C = C + 0.5 - delta[m8][m3] * dk[m][m8][m3][mm][nn][q - qtime + 1] * delta[mm][nn];											
									}
								}
								b[ib[m][is[mm]]][nn] = b[ib[m][is[mm]]][nn] + sum;
							}
							...
							for (int mm = 1; mm <= 8; mm++)
							{
								...
								for (int m3 = 1; m3 <= 3; m3++)
								{
									for (int m8 = 1; m8 <= 8; m8++)
									{
										...
										{
											sum = sum + delta[m8][m3] * dk[m][m8][m3][mm][nn][qtime];
											C = C + 0.5 * delta[m8][m3] * dk[m][m8][m3][mm][nn][qtime]* delta[mm][nn];								
										}
									}
								}
								b[ib[m][is[mm]]][nn] = b[ib[m][is[mm]]][nn] + sum;
					...
					for (int m3 = 1; m3 <= 3; m3++)
					{
						for (int m8 = 1; m8 <= 8; m8++)
						{
							if (phasechange[m] == 1) //if phase change at a pixel at x=nx surface
							{
								...
								{
									sum = sum + delta[m8][m3] * dk[m][m8][m3][mm][nn][qtime - pixt[m]]; //sum over history since formed
									C = C + 0.5 * delta[m8][m3] * dk[m][m8][m3][mm][nn][qtime - pixt[m]] * delta[mm][nn];									
								}
							}
							...
							if (phasechange[m] == 0) //if phase is always the same chemical
							{
								...
								{
									sum = sum + delta[m8][m3] * dk[m][m8][m3][mm][nn][qtime]; //sum over history
									C = C + 0.5 * delta[m8][m3] * dk[m][m8][m3][mm][nn][qtime] * delta[mm][nn];										
								}
							}
						}
					...
					for (int m3 = 1; m3 <= 3; m3++)
					{
						for (int m8 = 1; m8 <= 8; m8++)
						{
							if (phasechange[m] == 1)
							{
								...
								{
									sum = sum + delta[m8][m3] * dk[m][m8][m3][mm][nn][qtime - pixt[m]]; //sum over history since the phase is formed
									C = C + 0.5 * delta[m8][m3] * dk[m][m8][m3][mm][nn][qtime - pixt[m]] * delta[mm][nn];										
								}
							}
							...
							if (phasechange[m] == 0)
							{
								...
								for (qtime = 1; qtime <= q; qtime++)
								{
									sum = sum + delta[m8][m3] * dk[m][m8][m3][mm][nn][qtime]; //sum over history since the first image
									C = C + 0.5 * delta[m8][m3] * dk[m][m8][m3][mm][nn][qtime] * delta[mm][nn];										
								}
							}
						}
					...
					for (int m3 = 1; m3 <= 3; m3++)
					{
						for (int m8 = 1; m8 <= 8; m8++)
						{
							if (phasechange[m] == 1)
							{
								for (qtime = pixt[m] + 1; qtime <= q; qtime++)
								{
									sum = sum + delta[m8][m3] * dk[m][m8][m3][mm][nn][qtime - pixt[m]];
									C = C + 0.5 * delta[m8][m3] * dk[m][m8][m3][mm][nn][qtime - pixt[m]] * delta[mm][nn];										
								}
							}
			...
			for (int m = 1; m <= ns; m++)
			{
				for (int qtime = 1; qtime <= 1; qtime++)
				{
					gb[m][j] = gb[m][j] + u[ib[m][1]][q + 1 - qtime][n] *
						(dk[ib[m][27]][1][j][4][n][qtime] + dk[ib[m][7]][2][j][3][n][qtime] +
						dk[ib[m][25]][5][j][8][n][qtime] + dk[ib[m][15]][6][j][7][n][qtime]) +
						u[ib[m][2]][q + 1 - qtime][n] * (dk[ib[m][27]][1][j][3][n][qtime] + dk[ib[m][25]
						][5][j][7][n][qtime]) + u[ib[m][3]
						][q + 1 - qtime][n] * (dk[ib[m][27]][1][j][2][n][qtime] + dk[ib[m][5]
						][4][j][3][n][qtime] + dk[ib[m][13]
						][8][j][7][n][qtime] + dk[ib[m][25]
						][5][j][6][n][qtime]) + u[ib[m][4]
						][q + 1 - qtime][n] * (dk[ib[m][5]
						][4][j][2][n][qtime] + dk[ib[m][13]
						][8][j][6][n][qtime]) + u[ib[m][5]
						][q + 1 - qtime][n] * (dk[ib[m][6]
						][3][j][2][n][qtime] + dk[ib[m][5]
						][4][j][1][n][qtime] + dk[ib[m][14]
						][7][j][6][n][qtime] + dk[ib[m][13]
						][8][j][5][n][qtime]) + u[ib[m][6]
						][q + 1 - qtime][n] * (dk[ib[m][6]
						][3][j][1][n][qtime] + dk[ib[m][14]
						][7][j][5][n][qtime]) + u[ib[m][7]][q + 1 - qtime][n] * (dk[ib[m][6]
						][3][j][4][n][qtime] + dk[ib[m][7]
						][2][j][1][n][qtime] + dk[ib[m][14]
						][7][j][8][n][qtime] + dk[ib[m][15]
						][6][j][5][n][qtime]) + u[ib[m][8]
						][q + 1 - qtime][n] * (dk[ib[m][7]
						][2][j][4][n][qtime] + dk[ib[m][15]
						][6][j][8][n][qtime]) + u[ib[m][9]
						][q + 1 - qtime][n] * (dk[ib[m][25]
						][5][j][4][n][qtime] + dk[ib[m][15]
						][6][j][3][n][qtime]) + u[ib[m][10]
						][q + 1 - qtime][n] * (dk[ib[m][25]
						][5][j][3][n][qtime]) + u[ib[m][11]
						][q + 1 - qtime][n] * (dk[ib[m][13]
						][8][j][3][n][qtime] + dk[ib[m][25]
						][5][j][2][n][qtime]) + u[ib[m][12]
						][q + 1 - qtime][n] * (dk[ib[m][13]
						][8][j][2][n][qtime]) + u[ib[m][13]
						][q + 1 - qtime][n] * (dk[ib[m][13]
						][8][j][1][n][qtime] + dk[ib[m][14]
						][7][j][2][n][qtime]) + u[ib[m][14]
						][q + 1 - qtime][n] * (dk[ib[m][14]
						][7][j][1][n][qtime]) + u[ib[m][15]
						][q + 1 - qtime][n] * (dk[ib[m][14]
						][7][j][4][n][qtime] + dk[ib[m][15]
						][6][j][1][n][qtime]) + u[ib[m][16]
						][q + 1 - qtime][n] * (dk[ib[m][15]
						][6][j][4][n][qtime]) + u[ib[m][17]
						][q + 1 - qtime][n] * (dk[ib[m][27]
						][1][j][8][n][qtime] + dk[ib[m][7]
						][2][j][7][n][qtime]) + u[ib[m][18]
						][q + 1 - qtime][n] * (dk[ib[m][27]
						][1][j][7][n][qtime]) + u[ib[m][19]
						][q + 1 - qtime][n] * (dk[ib[m][27]
						][1][j][6][n][qtime] + dk[ib[m][5]
						][4][j][7][n][qtime]) + u[ib[m][20]
						][q + 1 - qtime][n] * (dk[ib[m][5]
						][4][j][6][n][qtime]) + u[ib[m][21]
						][q + 1 - qtime][n] * (dk[ib[m][5]
						][4][j][5][n][qtime] + dk[ib[m][6]
						][3][j][6][n][qtime]) + u[ib[m][22]
						][q + 1 - qtime][n] * (dk[ib[m][6]
						][3][j][5][n][qtime]) + u[ib[m][23]
						][q + 1 - qtime][n] * (dk[ib[m][6]
						][3][j][8][n][qtime] + dk[ib[m][7]
						][2][j][5][n][qtime]) + u[ib[m][24]
						][q + 1 - qtime][n] * (dk[ib[m][7]
						][2][j][8][n][qtime]) + u[ib[m][25]
						][q + 1 - qtime][n] * (dk[ib[m][14]
						][7][j][3][n][qtime] + dk[ib[m][13]
						][8][j][4][n][qtime] + dk[ib[m][15]
						][6][j][2][n][qtime] + dk[ib[m][25]
						][5][j][1][n][qtime]) + u[ib[m][26]
						][q + 1 - qtime][n] * (dk[ib[m][6]
						][3][j][7][n][qtime] + dk[ib[m][5]
						][4][j][8][n][qtime] + dk[ib[m][27]
						][1][j][5][n][qtime] + dk[ib[m][7]
						][2][j][6][n][qtime]) + u[ib[m][27]
						][q + 1 - qtime][n] * (dk[ib[m][27]
						][1][j][1][n][qtime] + dk[ib[m][7]
						][2][j][2][n][qtime] + dk[ib[m][6]
						][3][j][3][n][qtime] + dk[ib[m][5]
						][4][j][4][n][qtime] + dk[ib[m][25]
						][5][j][5][n][qtime] + dk[ib[m][15]
						][6][j][6][n][qtime] + dk[ib[m][14]
						][7][j][7][n][qtime] + dk[ib[m][13]
						][8][j][8][n][qtime]);
				}
			}
		}
		...
		for (int j = 1; j <= 3; j++)
		{
			for (int n = 1; n <= 3; n++)
			{
				for (int m = 1; m <= ns; m++)
				{
					...
					Ah[m][j] = Ah[m][j] + h[ib[m][1]][n] * (dk[ib[m][27]][1][j][4][n][qtime]
						+ dk[ib[m][7]][2][j][3][n][qtime]
						+ dk[ib[m][25]][5][j][8][n][qtime] + dk[ib[m][15]][6][j][7][n][qtime]) +
						h[ib[m][2]][n] * (dk[ib[m][27]][1][j][3][n][qtime]
						+ dk[ib[m][25]][5][j][7][n][qtime]) +
						h[ib[m][3]][n] * (dk[ib[m][27]][1][j][2][n][qtime] + dk[ib[m][5]][4][j][3][n][qtime] +
						dk[ib[m][13]][8][j][7][n][qtime] + dk[ib[m][25]][5][j][6][n][qtime]) +
						h[ib[m][4]][n] * (dk[ib[m][5]][4][j][2][n][qtime]
						+ dk[ib[m][13]][8][j][6][n][qtime]) +
						h[ib[m][5]][n] * (dk[ib[m][6]][3][j][2][n][qtime] + dk[ib[m][5]][4][j][1][n][qtime] +
						dk[ib[m][14]][7][j][6][n][qtime] + dk[ib[m][13]][8][j][5][n][qtime]) +
						h[ib[m][6]][n] * (dk[ib[m][6]][3][j][1][n][qtime]
						+ dk[ib[m][14]][7][j][5][n][qtime]) +
						h[ib[m][7]][n] * (dk[ib[m][6]][3][j][4][n][qtime] + dk[ib[m][7]][2][j][1][n][qtime] +
						dk[ib[m][14]][7][j][8][n][qtime] + dk[ib[m][15]][6][j][5][n][qtime]) +
						h[ib[m][8]][n] * (dk[ib[m][7]][2][j][4][n][qtime]
						+ dk[ib[m][15]][6][j][8][n][qtime]) +
						h[ib[m][9]][n] * (dk[ib[m][25]][5][j][4][n][qtime]
						+ dk[ib[m][15]][6][j][3][n][qtime]) +
						h[ib[m][10]][n] * (dk[ib[m][25]][5][j][3][n][qtime]) +
						h[ib[m][11]][n] * (dk[ib[m][13]][8][j][3][n][qtime]
						+ dk[ib[m][25]][5][j][2][n][qtime]) +
						h[ib[m][12]][n] * (dk[ib[m][13]][8][j][2][n][qtime]) +
						h[ib[m][13]][n] * (dk[ib[m][13]][8][j][1][n][qtime]
						+ dk[ib[m][14]][7][j][2][n][qtime]) +
						h[ib[m][14]][n] * (dk[ib[m][14]][7][j][1][n][qtime]) +
						h[ib[m][15]][n] * (dk[ib[m][14]][7][j][4][n][qtime]
						+ dk[ib[m][15]][6][j][1][n][qtime]) +
						h[ib[m][16]][n] * (dk[ib[m][15]][6][j][4][n][qtime]) +
						h[ib[m][17]][n] * (dk[ib[m][27]][1][j][8][n][qtime]
						+ dk[ib[m][7]][2][j][7][n][qtime]) +
						h[ib[m][18]][n] * (dk[ib[m][27]][1][j][7][n][qtime]) +
						h[ib[m][19]][n] * (dk[ib[m][27]][1][j][6][n][qtime]
						+ dk[ib[m][5]][4][j][7][n][qtime]) +
						h[ib[m][20]][n] * (dk[ib[m][5]][4][j][6][n][qtime]) +
						h[ib[m][21]][n] * (dk[ib[m][5]][4][j][5][n][qtime]
						+ dk[ib[m][6]][3][j][6][n][qtime]) +
						h[ib[m][22]][n] * (dk[ib[m][6]][3][j][5][n][qtime]) +
						h[ib[m][23]][n] * (dk[ib[m][6]][3][j][8][n][qtime]
						+ dk[ib[m][7]][2][j][5][n][qtime]) +
						h[ib[m][24]][n] * (dk[ib[m][7]][2][j][8][n][qtime]) +
						h[ib[m][25]][n] * (dk[ib[m][14]][7][j][3][n][qtime]
						+ dk[ib[m][13]][8][j][4][n][qtime] +
						dk[ib[m][15]][6][j][2][n][qtime] + dk[ib[m][25]][5][j][1][n][qtime]) +
						h[ib[m][26]][n] * (dk[ib[m][6]][3][j][7][n][qtime]
						+ dk[ib[m][5]][4][j][8][n][qtime] +
						dk[ib[m][27]][1][j][5][n][qtime] + dk[ib[m][7]][2][j][6][n][qtime]) +
						h[ib[m][27]][n] * (dk[ib[m][27]][1][j][1][n][qtime]
						+ dk[ib[m][7]][2][j][2][n][qtime] +
						dk[ib[m][6]][3][j][3][n][qtime] + dk[ib[m][5]][4][j][4][n][qtime]
						+ dk[ib[m][25]][5][j][5][n][qtime] +
						dk[ib[m][15]][6][j][6][n][qtime] + dk[ib[m][14]][7][j][7][n][qtime] +
						dk[ib[m][13]][8][j][8][n][qtime]);
					...
					for (int qtime = set; qtime <= q; qtime++)
					{
						...
						for (int n3 = 1; n3 <= 3; n3++)
						{
							for (int n8 = 1; n8 <= 8; n8++)
							{
								for (int n = 1; n <= 6; n++)
								{
									str11 = str11 + cmod[m][1][n][q + 1 - qtime] * es[n][n8][n3] * uu[n8][qtime][n3];										  										  
									str22 = str22 + cmod[m][2][n][q + 1 - qtime] * es[n][n8][n3] * uu[n8][qtime][n3];																				  
									str33 = str33 + cmod[m][3][n][q + 1 - qtime] * es[n][n8][n3] * uu[n8][qtime][n3];																				  
									str13 = str13 + cmod[m][4][n][q + 1 - qtime] * es[n][n8][n3] * uu[n8][qtime][n3];										  										  
									str23 = str23 + cmod[m][5][n][q + 1 - qtime] * es[n][n8][n3] * uu[n8][qtime][n3];									  
									str12 = str12 + cmod[m][6][n][q + 1 - qtime] * es[n][n8][n3] * uu[n8][qtime][n3];										  										  
								}
							}
						}
					}
		...
		for (int j = 1; j <= ny; j++)
		{
			for (int i = 1; i <= nx; i++)
			{
				...
				if (pix[m] == 12){
					...
					double temp_mod = csh_modulus(str11,str22,str33,str13,str23,str12); 
					csh_mod.push_back({m,temp_mod});
				}
