...
//YZ:added 200212
std::ofstream stress_fout("stress_5csh_bulk_properties.txt"); // store stresses of each CSH after equilibrium of each step
std::ofstream phasechange_fout("phasechange_5csh_bulk_properties.txt"); // store phase change at which pixel
std::ofstream phasechange_csh_fout("phasechange_5csh_bulk_csh_properties.txt"); // store phase change at which pixel (only for CSH)
std::ofstream fout("outputfile_5csh_bulk_properties.txt"); // store the bulk stress due to applied strains

//global variables: 100x100x100 voxels, assume 50 input files, 3 directions
float u[1000001][51][4], gb[1000001][4], b[1000001][4]; //node displacement, gradient, b value
float h[1000001][4], Ah[1000001][4]; // for solving the gb+b=0
float cmod[101][7][7][51], dk[101][9][4][9][4][51]; //modulus matrix
...
float strxx, stryy, strzz, strxz, stryz, strxy; //averaged global stresses
float sxx, syy, szz, sxz, syz, sxy; //averaged global strains
float exx, eyy, ezz, exz, eyz, exy; //applied global strains
float C; //constant C

float str11, str22, str33, str12, str23, str13; //pixel stresses
float s11, s22, s33, s12, s23, s13; //pixel strains
float dxx, dyy, dzz, dxy, dxz, dyz; //to get the strain in one pixel

//YZ: added 200512
float phasemod[101][51][3]; //phasemod[phase][time step][bulk/shear single value]
float prob[101]; //prob[phase], volume fraction of each phase
int in[28]; //MANUAL table 3: use delta i, delta j, delta k to construct 27 neighbors of 1 element
int jn1[28];
int kn[28];

int nphase;//nphase value should not be zero. otherwise change it in ppixel function
int step; //step number should be equal to number of Thames microstructure images

float t[51]; //time step should be set accordingly to Thames microstructure images
...
//////////////////////////////////////////////////
//YZ added 200515
// random index for 5 groups of csh
std::vector<int> index1={66, 19, 86, 57, 30,  6, 84, 17, 10, 45, 14, 36, 56, 50, 31, 60, 92, 29, 16, 32,  1};
std::vector<int> index2={88,  2, 68, 54, 69, 95, 72, 12, 80, 40, 99, 74, 65, 94, 25, 83, 70, 35, 71, 77, 76,  3,  4, 22, 62, 82, 18, 47, 78, 41, 28};
std::vector<int> index3={42, 20, 58, 73, 91, 55, 34, 52, 59, 96, 33, 67,  9, 90, 44, 89, 98,  5, 11, 43, 15, 75, 87};
std::vector<int> index4={23, 79, 49, 13, 48, 27, 21, 81, 63, 100,53,  8,  7, 38, 61, 93, 64, 46};
std::vector<int> index5={24, 26, 37, 39, 51, 85, 97};

//////////////////////////////////////////////////
float Fmodulus_22(float t);
float Fmodulus_23(float t);
float Fmodulus_24(float t);
float Fmodulus_25(float t);
float Fmodulus_26(float t);
...
//YZ: construct materials: phase num, time steps, phasemod
void materials(){
	nphase = 27;//nphase value should not be zero. otherwise change it in ppixel function
	step = 27; //step number should be equal to number of Thames microstructure images
	
	//time step should be set accordingly to Thames microstructure images
	t[1]=0.0;
	t[2]=0.1834;
	t[3]=0.4004;
	t[4]=0.49;
	...
	ny = 100;
	nz = 100;
	ns = nx * ny * nz; //total number of sites
	...
	*/
	...
	//YZ added 0515, use random_sample.r to generate random index for 5 groups
	//phase 22-26 are CSH groups
	phasemod[22][1][1] = 19; //group1 (phase 22), probability = 21%, E=19
	phasemod[22][1][2] = 0.25;
	phasemod[23][1][1] = 23; //group2 (phase 23), probability = 31%, E=23
	phasemod[23][1][2] = 0.25;
	phasemod[24][1][1] = 27; //group3 (phase 24), probability = 23%, E=27
	phasemod[24][1][2] = 0.25;
	phasemod[25][1][1] = 31; //group4 (phase 25), probability = 18%, E=31
	phasemod[25][1][2] = 0.25;
	phasemod[26][1][1] = 35; //group5 (phase 26), probability = 7%, E=35
	phasemod[26][1][2] = 0.25;
	...
	for (int i = 2; i <= step; i++)
	{	
		phasemod[22][i][1] = Fmodulus_22(t[i])-Fmodulus_22(t[i-1]); //Fmodulus(t[i]-t[1]); // Fmodulus(t[i] - t[1])-Fmodulus(t[i-1] - t[1]); //TODO, how to update for each CSH pixel that appears at each time step????
		phasemod[23][i][1] = Fmodulus_23(t[i])-Fmodulus_23(t[i-1]);
		phasemod[24][i][1] = Fmodulus_24(t[i])-Fmodulus_24(t[i-1]);
		phasemod[25][i][1] = Fmodulus_25(t[i])-Fmodulus_25(t[i-1]);
		phasemod[26][i][1] = Fmodulus_26(t[i])-Fmodulus_26(t[i-1]);
	}
	...
	*/
	exx = -0.005; // triaxial compression
	eyy = -0.005;
	ezz = -0.005;
	exz = 0.0;
	eyz = 0.0;
	...
	in[25] = 0; jn1[25] = 0; kn[25] = -1;
	in[26] = 0; jn1[26] = 0; kn[26] = 1;
	in[27] = 0; jn1[27] = 0; kn[27] = 0;
	...
	for (int k = 1; k <= nz; k++)
	{
		for (int j = 1; j <= ny; j++)
		{
			for (int i = 1; i <= nx; i++)
			{
				...
				if (pix[m] == 12) { // CSH phase, group to one of 22,23,24,25,26
					int temp = m % 100 + 1; // map m to 1~100

					if (std::count(index1.begin(),index1.end(),temp)) pix[m] = 22; // group 1
					else if (std::count(index2.begin(),index2.end(),temp)) pix[m] = 23; // group 2
					else if (std::count(index3.begin(),index3.end(),temp)) pix[m] = 24; // group 3
					else if (std::count(index4.begin(),index4.end(),temp)) pix[m] = 25; // group 4
					else pix[m] = 26; // group 5
				}
			}
		}
	}
...
void assig(int ns, int nphase, float (&prob)[101]) //float prob[101] stores volume fraction of each phase
{	
...
{
	float dndx[9], dndy[9], dndz[9]; //dndx, dndy, dndz are strain components in 3 directions (x,y,z) of 8 nodes
	//each pixel has 6 independent deformation gradient compuonents, 8 nodes, 3 directions
	float es[7][9][4]; //es is the displacement/deformation gradient that relates nodal displacement to strains, dndx = es * uu
	float uu[9][4]; //uu is the displacement vectors of 8 nodes
...
*/
void femat(int nx, int ny, int nz, int ns, float (&phasemod)[101][51][3], int nphase, int q)
{
	std::cout << "  Compute stiffness matrix dk, and b, C." << std::endl;

	float dndx[9], dndy[9], dndz[9]; //derivatives of shape functions
	float ck[7][7], cmu[7][7]; //bulk and shear matrix
	float g[4][4][4]; //weight vector of Simpson's rule
	float es[7][9][4]; //deformation gradient
	float delta[9][4]; 
...
void energy(int nx, int ny, int nz, int ns, float& utot, int q)
{	
...
void dembx(int ns, int &Lstep, float &gg, float(&dk)[101][9][4][9][4][51], float &gtest, int &ldemb, int &kkk, int q)
{
...
void stress(int nx, int ny, int nz, int ns, int q)
{
	...
	float dndx[9], dndy[9], dndz[9], es[7][9][4], uu[9][51][4];
...
float Fmodulus_22(float t) {
	return 19/2 * (1 + exp(-0.2 * t));
}
float Fmodulus_23(float t) {
	return 23/2 * (1 + exp(-0.2 * t));
}
float Fmodulus_24(float t) {
	return 27/2 * (1 + exp(-0.2 * t));
}
float Fmodulus_25(float t) {
	return 31/2 * (1 + exp(-0.2 * t));
}
float Fmodulus_26(float t) {
	return 35/2 * (1 + exp(-0.2 * t));
}

//////////////////////////////////////////////////////
void properties(char filename[],string prefilename[], int img_ind)
{	
	int q = 1;
	std::cout << '\n';
	std::cout << "Time step: " << img_ind << std::endl;

	//read in microstructure file starting from 1.txt
	strcpy(filename, prefilename[img_ind].c_str());
	ifstream fin(filename);
	...
	*/
	float gtest = pow(10.0, -8) * ns; //modified: 10^(-12)
	...
	*/
	...
	for (int k = 1; k <= nz; k++)
	{
		for (int j = 1; j <= ny; j++)
		{
			...
			{
				int	m = nx * ny * (k - 1) + nx * (j - 1) + i;
				float x = 1.0 * (i - 1); //YG: 1.0 is the size of 1 pixel, 1um. x,y,z are coordinates of each node
				float y = 1.0 * (j - 1);
				float z = 1.0 * (k - 1);
	...
	float gg = 0.0; //gg is the norm squared of the gradient (gg=gb*gb)
	...
	<< ", Volume fractions of CSH: " << prob[22] << " " << prob[23] << " " << prob[24] << " " << prob[25] << " " << prob[26] << std::endl; 
...
{	
	...
	*/
	...
	string prefilename[27]; //file number should be equal to number of Thames microstructure images
	char filename[15];
	...
	prefilename[10] = "10.img";
	prefilename[11] = "11.img";
	prefilename[12] = "12.img";	
	prefilename[13] = "13.img";
	prefilename[14] = "14.img";
	prefilename[15] = "15.img";
	prefilename[16] = "16.img";
	prefilename[17] = "17.img";
	...
	/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! TIME LOOP Starts !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
	/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
	materials(); // set up materials properties and time steps
	for (int ind=1; ind <=26; ind++) {
		resetall();
		
		properties(filename, prefilename, ind);		
	}
