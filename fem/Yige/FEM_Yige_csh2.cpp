...
//////////////////////////////////////////////////
//YZ: added 200512
std::vector < std::pair<int, float> > csh_mod;// matrix to save modulus of csh at each time step
//YZ: added 200512
float csh_modulus(float str11, float str22,float str33,float str13,float str23,float str12) {
	float str_mean = (str11+str22+str33+str13+str23+str12)/6;
	float ind = 1/(15-str_mean)-1/25; // when str_mean = -10, ind=0, when str_mean = 15, ind=infinity
	float csh_mod = 11.2 + 11.2 * exp(-0.2 * ind);
	return csh_mod;
}

float Fmodulus(float t);
//YZ: construct materials: phase num, time steps, phasemod
void materials(){
	nphase = 22;//nphase value should not be zero. otherwise change it in ppixel function
